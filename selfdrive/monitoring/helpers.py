...
class DRIVER_MONITOR_SETTINGS:
  def __init__(self, device_type):
    ...
    self._AWARENESS_TIME = 30.0  # passive wheeltouch total timeout
    self._AWARENESS_PRE_TIME_TILL_TERMINAL = 15.0
...
class DriverMonitoring:
  def __init__(self, rhd_saved=False, settings=None, always_on=False, awareness_shutoff=False):
    ...
    self.driver_distraction_filter = FirstOrderFilter(0.0, self.settings._DISTRACTED_FILTER_TS, self.settings._DT_DMON)
  ...
  def _set_policy(self, brake_disengage_prob, car_speed):
    ...
    self.pose.cfactor_pitch = (
      np.interp(bp_normal, [0, 0.5], [self.settings._POSE_PITCH_THRESHOLD_SLACK, self.settings._POSE_PITCH_THRESHOLD_STRICT])
      / self.settings._POSE_PITCH_THRESHOLD
    )
    self.pose.cfactor_yaw = (
      np.interp(bp_normal, [0, 0.5], [self.settings._POSE_YAW_THRESHOLD_SLACK, self.settings._POSE_YAW_THRESHOLD_STRICT]) / self.settings._POSE_YAW_THRESHOLD
    )
  ...
  def _get_distracted_types(self):
    ...
    if not self.pose.calibrated:
      ...
      pitch_error = self.pose.pitch - min(max(self.pose.pitch_offseter.filtered_stat.mean(), self.settings._PITCH_MIN_OFFSET), self.settings._PITCH_MAX_OFFSET)
      yaw_error = self.pose.yaw - min(max(self.pose.yaw_offseter.filtered_stat.mean(), self.settings._YAW_MIN_OFFSET), self.settings._YAW_MAX_OFFSET)
    pitch_error = 0 if pitch_error > 0 else abs(pitch_error)  # no positive pitch limit

    if yaw_error * self.pose.steer_yaw_offset > 0:  # unidirectional
      yaw_error = max(abs(yaw_error) - min(abs(self.pose.steer_yaw_offset), self.settings._POSE_YAW_STEER_MAX_OFFSET), 0.0)
    else:
  ...
  def _update_states(self, driver_state, cal_rpy, car_speed, op_engaged, standstill, demo_mode=False, steering_angle_deg=0.0):
    rhd_pred = driver_state.wheelOnRightProb
    # calibrates only when there's movement and either face detected
    if car_speed > self.settings._WHEELPOS_CALIB_MIN_SPEED and (
      driver_state.leftDriverData.faceProb > self.settings._FACE_THRESHOLD or driver_state.rightDriverData.faceProb > self.settings._FACE_THRESHOLD
    ):
    ...
    if not all(len(x) > 0 for x in (driver_data.faceOrientation, driver_data.facePosition, driver_data.faceOrientationStd, driver_data.facePositionStd)):
      return

    self.face_detected = driver_data.faceProb > self.settings._FACE_THRESHOLD
    self.pose.roll, self.pose.pitch, self.pose.yaw = face_orientation_from_net(driver_data.faceOrientation, driver_data.facePosition, cal_rpy)
    steer_d = max(abs(steering_angle_deg) - self.settings._POSE_YAW_MIN_STEER_DEG, 0.0)
    ...
    self.driver_distracted = (
      ...
      and driver_data.faceProb > self.settings._FACE_THRESHOLD
    ...
    self.pose.calibrated = (
      self.pose.pitch_offseter.filtered_stat.n > self.settings._POSE_OFFSET_MIN_COUNT
      and self.pose.yaw_offseter.filtered_stat.n > self.settings._POSE_OFFSET_MIN_COUNT
    )
  ...
  def _update_events(self, driver_engaged, op_engaged, standstill, wrong_gear, car_speed):
    ...
    if (
      (driver_engaged and self.awareness > 0 and not self.active_monitoring_mode)
      or (not always_on_valid and not op_engaged)
      or (always_on_valid and not op_engaged and self.awareness <= 0)
    ):
    ...
    if self.awareness > 0 and ((self.driver_distraction_filter.x < 0.37 and self.face_detected and self.pose.low_std) or standstill_orange_exemption):
      ...
      self.awareness = min(
        self.awareness
        + ((self.settings._RECOVERY_FACTOR_MAX - self.settings._RECOVERY_FACTOR_MIN) * (1.0 - self.awareness) + self.settings._RECOVERY_FACTOR_MIN)
        * self.step_change,
        1.0,
      )
      if self.awareness == 1.0:
        self.awareness_passive = min(self.awareness_passive + self.step_change, 1.0)
  ...
  def run_step(self, sm, demo=False):
    ...
    self._update_events(driver_engaged=driver_engaged, op_engaged=enabled, standstill=standstill, wrong_gear=wrong_gear, car_speed=highway_speed)
