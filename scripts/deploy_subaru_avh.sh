#!/usr/bin/env bash
# Deploy Subaru AVH brake-hold changes to comma device in a single reboot.
#   1. Preflight (local sources present, DisableUpdates=1)
#   2. Build panda firmware locally
#   3. Run Subaru brake-hold safety tests
#   4. Rsync sources to device
#   5. Verify md5sums for key files + params_pyx build inputs
#   6. Rebuild params_pyx.so on device (needed for SubaruAutoVehicleHold key)
#   7. Smoke-test new .so before swapping
#   8. Atomic swap of params_pyx.so
#   9. Set SubaruAutoVehicleHold=1, DisableUpdates=1, overlay-swap guards
#  10. Pre-reboot import + read check
#  11. Single reboot
#  12. Post-reboot verification: firmware md5 + AVH param + brake-hold imports
set -euo pipefail
...
# Files md5-verified after rsync
VERIFY_FILES=(
  "$PANDA_BIN"
...
rsync -avz \
  ...
  --exclude='.ruff_cache/' --exclude='.worktrees/' --exclude='.antigravitycli/' \
  --exclude='build/' --exclude='list/' --exclude='logs/' \
  --exclude='openpilot/' --exclude='temp/' --exclude='*.pyc' \
  --include='*/' \
...
# ── 7. smoke-test new params_pyx.so before swapping ───────────────────────────
...
p = Params()
_ = p.get_default_value('SubaruAutoVehicleHold')  # new key must be compiled in (no UnknownKeyName)
...
print('  smoke test OK — SubaruAutoVehicleHold recognised, Params() path correct')
\""
...
from openpilot.common.params import Params
p = Params()
print('  SubaruAutoVehicleHold runtime:', p.get_bool('SubaruAutoVehicleHold'))
